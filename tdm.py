...
def run(model,train_set,test_set,model_save_path,train_step, validation_step,model_pb_save_path):
    ...
    with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
      ...
      for _ in range(1):
        ...
        for i in range(train_step):
          ...
          loss_sum += loss
          if model.global_step.eval() % (train_step//3) == 0:
            ...
            if best_auc < auc:
                ...
                constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def,
                                                                           ['input', 'prediction'])
                with tf.gfile.FastGFile(model_pb_save_path,
                                        mode='wb') as f:  # 模型的名字是model.pb
                    f.write(constant_graph.SerializeToString())
                upload_result_to_hdfs("/user/dev/andrew.zhu/tdm/model/tdm.pb",
                                      model_pb_save_path)
...
def main():
    ...
    model_pb_save_path = '/home/dev/data/andrew.zhu/tdm/model/tdm.pb'
    train_dir = '/home/dev/data/andrew.zhu/tdm/data_flow/train'
    test_dir = '/home/dev/data/andrew.zhu/tdm/data_flow/test'
    model_feature_dim = 9
    data_generator_feature_dim = 10
    model = Model(item_size, node_size, model_feature_dim)
    batch_size = 20000
    while num_epoch > 0:
        ...
        data_generator = DataGenerator(train_dir, test_dir, data_generator_feature_dim, batch_size=batch_size)
        ...
        run(model,train,test,model_save_path,train_step,validation_step,model_pb_save_path)
        num_epoch -= 1
